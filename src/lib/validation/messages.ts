...
export function createValidationMessages(t: TranslateFn) {
  return {
    ...
    minCharacters: (field: string, count: number) => t("validation.minCharacters", { field, count }),
    maxCharacters: (field: string, count: number) => t("validation.maxCharacters", { field, count }),
    ...
    nonNegativeNumber: (field: string) => t("validation.nonNegativeNumber", { field }),
  };
}
